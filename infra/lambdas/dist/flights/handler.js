...
async function searchFlights(body) {
    ...
    const origin = typeof data.origin === 'object' ? data.origin.code : data.origin;
    const destination = typeof data.destination === 'object' ? data.destination.code : data.destination;
    const outbound = await (0, db_1.query)(`SELECT ${FLIGHT_SELECT}
     ...
     ORDER BY f.departure_time`, [origin.toUpperCase(), destination.toUpperCase(), data.date, cabinClass, passengers]);
    ...
    if (data.return_date) {
       ...
       ${FLIGHT_GROUP}
       HAVING COUNT(s.id) FILTER (WHERE s.is_occupied = false AND s.class = $4) >= $5
       ORDER BY f.departure_time`, [origin.toUpperCase(), destination.toUpperCase(), data.return_date, cabinClass, passengers]);
