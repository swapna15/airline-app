...
async function lookupPassenger(
  ...
  if (!pnr && !(lastName && flightNumber)) {
    return badRequest('Provide pnr, or both last_name and flight_number');
  }
  ...
  if (!rows.length) return notFound('Passenger');
  return ok(rows);
}
...
async function checkIn(
  ...
  const data = parseBody<CheckInInput>(body);
  if (!data?.booking_passenger_id) {
    return badRequest('booking_passenger_id is required');
  }
  ...
  const passenger = await queryOne<{
    ...
    `SELECT b.id AS booking_id, b.status AS booking_status, f.departure_time
     FROM booking_passengers bp
     JOIN bookings b ON b.id = bp.booking_id
     JOIN flights f ON f.id = b.flight_id
     WHERE bp.id = $1`,
    [data.booking_passenger_id],
  );

  if (!passenger) return notFound('Passenger booking');
  if ((passenger as any).booking_status === 'cancelled') {
    return badRequest('Cannot check in a cancelled booking');
  }

  // Check if already checked in
  const existing = await queryOne(
    `SELECT id, status FROM checkins WHERE booking_passenger_id = $1`,
    [data.booking_passenger_id],
  );
  if (existing && (existing as any).status === 'checked_in') {
    return badRequest('Passenger is already checked in');
  }

  // Verify check-in window (24h before, close 45min before departure)
  const departure = new Date((passenger as any).departure_time);
  const now = new Date();
  const hoursUntil = (departure.getTime() - now.getTime()) / 3_600_000;
  if (hoursUntil > 24) return badRequest('Check-in opens 24 hours before departure');
  if (hoursUntil < 0.75) return badRequest('Check-in closed 45 minutes before departure');
  ...
  const checkin = await queryOne(
    `INSERT INTO checkins
     ...
     RETURNING id, status, baggage_count, checked_in_at`,
    [data.booking_passenger_id, agentId, data.baggage_count ?? 0, data.special_requests ?? null],
  );
...
async function getBoardingPass(checkinId: string): Promise<APIGatewayProxyResult> {
  ...
  if (!pass) return notFound('Check-in record');
  if ((pass as any).status !== 'checked_in') return badRequest('Passenger is not checked in');

  return ok(pass);
}

// ── GET /checkin/flight/{flightId} ────────────────────────────────────────────
async function getFlightCheckins(flightId: string): Promise<APIGatewayProxyResult> {
  ...
  const checkedIn = passengers.filter((p: any) => p.checkin_status === 'checked_in').length;

  return ok({ total, checked_in: checkedIn, passengers });
}
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const agentId: string = (event.requestContext as any)?.authorizer?.userId ?? '';

    if (!CHECKIN_ROLES.includes(requestorRole)) return forbidden();
    ...
    if (method === 'POST' && path === '/checkin') return checkIn(event.body, agentId);

    if (method === 'GET' && checkinId && path.endsWith('/boarding-pass')) {
      return getBoardingPass(checkinId);
    }

    if (method === 'GET' && flightId) return getFlightCheckins(flightId);
