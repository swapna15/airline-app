...
async function getGateFlights(date?: string, airport?: string): Promise<APIGatewayProxyResult> {
...
// ── GET /gate/flights/{id} ────────────────────────────────────────────────────
async function getFlightDetail(flightId: string): Promise<APIGatewayProxyResult> {
...
async function updateFlightStatus(
  ...
  const data = parseBody<StatusUpdateInput>(body);
  if (!data?.status) return badRequest('status is required');
  if (!VALID_STATUSES.includes(data.status)) {
    return badRequest(`status must be one of: ${VALID_STATUSES.join(', ')}`);
  }

  const flight = await queryOne<{ id: string; status: FlightStatus; departure_time: string }>(
    'SELECT id, status, departure_time FROM flights WHERE id = $1',
    [flightId],
  );
  if (!flight) return notFound('Flight');

  const currentStatus = (flight as any).status as FlightStatus;
  const allowed = ALLOWED_TRANSITIONS[currentStatus];
  if (!allowed.includes(data.status)) {
    return badRequest(`Cannot transition from '${currentStatus}' to '${data.status}'`);
  }

  // Build update fields
  const updates: string[] = ['status = $2', 'updated_at = now()'];
  const params: unknown[] = [flightId, data.status];

  if (data.gate) { params.push(data.gate); updates.push(`gate = $${params.length}`); }
  if (data.terminal) { params.push(data.terminal); updates.push(`terminal = $${params.length}`); }
  ...
  const updated = await queryOne(
    `UPDATE flights SET ${updates.join(', ')} WHERE id = $1
     RETURNING id, flight_number, status, gate, terminal, departure_time, arrival_time`,
    params,
  );
...
// ── POST /gate/flights/{id}/board ─────────────────────────────────────────────
interface BoardPassengerInput {
  checkin_id: string;
}
...
async function boardPassenger(
  ...
  const data = parseBody<BoardPassengerInput>(body);
  if (!data?.checkin_id) return badRequest('checkin_id is required');

  // Verify the checkin belongs to this flight
  const checkin = await queryOne<{ status: string; flight_id: string }>(
    `SELECT ci.status, b.flight_id
     FROM checkins ci
  ...
  if (!checkin) return notFound('Check-in record');
  if ((checkin as any).flight_id !== flightId) return badRequest('Check-in does not belong to this flight');
  if ((checkin as any).status === 'boarded') return badRequest('Passenger has already boarded');
  if ((checkin as any).status !== 'checked_in') return badRequest('Passenger is not checked in');

  const updated = await queryOne(
    `UPDATE checkins SET status = 'boarded', updated_at = now()
...
async function getManifest(flightId: string): Promise<APIGatewayProxyResult> {
  const manifest = await query(
    `SELECT
     ...
     ORDER BY s.seat_number NULLS LAST, bp.last_name`,
    [flightId],
  );

  const flight = await queryOne(
    `SELECT f.flight_number, f.departure_time, f.status, f.gate, f.terminal,
  ...
  return ok({
    ...
    boarded: manifest.filter((p: any) => p.checkin_status === 'boarded').length,
    checked_in: manifest.filter((p: any) => p.checkin_status === 'checked_in').length,
    not_checked_in: manifest.filter((p: any) => !p.checkin_status).length,
    passengers: manifest,
  });
}
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if (!GATE_ROLES.includes(requestorRole)) return forbidden();
    ...
    if (method === 'GET' && flightId) return getFlightDetail(flightId);
